...
//点击详情后scope.row作为事件参数传递，用sessionDetail接收单条咨询详情显示弹窗上面的用户、开始时间等
const sessionDetail=ref({})

const loadingMessages = ref(false) //点击详情时显示加载状态

const showDetailDialog = ref(false)  //控制详情弹窗关闭打开
...
const viewSessionDetail = (row) =>{
    ...
    getSessionDetail(row.id).then(res=>{  //向后端请求此条对话消息数据
        //请求成功后加载数据
...
const handleSearch =()=>{
    ...
    getConsulationPage(pagination).then(res=>{
        const {records,total}=res //后端返回的数据解构一下
