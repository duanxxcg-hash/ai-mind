<template>
    <div>
        ...
        <el-table :data="tableData" style="width: 100%;">
            ...
            <el-table-column prop="moodScreRange" label="情绪评分">
                <template #default="scope">
...
const formItem=[
    ...
    {comp:'select',prop:'moodScreRange',label:'情绪评分',placeholder:'请输入请选择评分范围',options:[{
        label:'低分(1-3)',
        value:'1-3'
    },
    {   
...
const detailDialogVisible = ref(false)//控制弹窗
const currentDetail = ref(null)  //当前行的详情对象
const aiData = ref(null)

//点击详情 打开弹窗 加载数据
const viewSessionDetail = (row)=>{
    currentDetail.value = row   //当前行的详情数据
    if(row.aiEmotionAnalysis){
        aiData.value = JSON.parse(row.aiEmotionAnalysis) //aiEmotionAnalysis是JSON字符串 将它转为字符串对象赋给aiData
    }else{
        aiData.value = {}
    }
