    ...
    <div class="dashboard-container">
                ...
                <el-card v-if="aiData.systemOverview">
                    <div class="card-content">
                        ...
                        <div class="info">
                            ...
                            <p class="subtitle-title">情绪健康指数{{ aiData.systemOverview.totalSessions}}</p>
                        </div>
                    </div>
                </el-card>
            </el-col>
        ...
        <el-row :gutter="20" style="margin-top: 20px;">
            ...
            <el-col :span="12">
              <el-card style="width: 100%;">
                    <template #header>
                        <div class="card-header">咨询活动统计</div>
                    </template>
                    <div class="chart-content">
                      ...
                      <div ref="consoltationChartRef" style="width: 100%; height: 260px;"></div>
                    </div>
                </el-card>
            </el-col>
        </el-row>
...
let consoltationChart = null
let userActivityChart = null

//组件实例 调用DOM组件的方法
const emotionChartRef = ref(null)
const consoltationChartRef = ref(null)
const userActivityChartRef = ref(null)

//
const initCharts = ()=>{
  initEmotionCharts()
  initconsolationCharts()
  inituserActivityCharts()
}

//初始化图表
const initEmotionCharts = ()=>{
  if(!emotionChartRef.value) return //DOM都不存在 就不创建了
  //如果有旧图表存在 销毁
  if(emotionChart){
    emotionChart.dispose()
  }

  //创建新图表 挂载在DOM元素emotionChartRef.value上
  ...
  const option = {
    ...
    xAxis:{
      type:'category',
      data:TrendData.map(item=>item.date), //遍历数组 将数组元素item.data组成的新数组赋值给data
      axisLine:{
        lineStyle:{
          color:'#2d3436'
        }
  ...
  emotionChart.setOption(option) //使用配置
  
}

//会话统计图表初始化
const initconsolationCharts = ()=>{
  if(!consoltationChartRef.value) return //DOM都不存在 就不创建了
  //如果有旧图表存在 销毁
  if(consoltationChart){
    consoltationChart.dispose()
  }

  //创建图表实例
  consoltationChart = echarts.init(consoltationChartRef.value)
  ...
  consoltationChart.setOption(option) //使用配置
  
}

//用户活跃度图表初始化
const inituserActivityCharts = ()=>{
  if(!userActivityChartRef.value) return //DOM都不存在 就不创建了
  //如果有旧图表存在 销毁
  ...
  userActivityChart.setOption(option) //使用配置
  
}

//aiData接受后端返回的数据分析所有数据
const aiData = ref({})
